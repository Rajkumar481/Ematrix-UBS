...
const Buyer = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (editMode) {
                await axios.patch(`http://localhost:3000/company/${currentBuyerId}`, formData);
            ...
            <table className="min-w-full table-auto border-collapse">
                <thead>
                    <tr className="bg-gray-100 text-center">
                        {['S.No', 'Company Name', 'Email', 'Phone', 'Address', 'GST', 'Actions'].map((col) => (
                    ...
                    {buyerData.map((buyer, index) => (
                        <tr key={buyer._id} className="text-center border-t">
                            ...
                            <td className="px-4 py-2 border space-x-2">
                                <button onClick={() => handleEdit(buyer)} className="text-blue-500"><FaEdit /></button>
                                <button onClick={() => handleDelete(buyer)} className="text-red-500"><FaTrash /></button>
                            </td>
                        </tr>
                    ))}
                </tbody>
            ...
            {openForm && (
                <div className="fixed inset-0 flex items-center justify-center bg-opacity-40 z-50">
                    <div className="bg-white p-6 rounded-lg w-full max-w-md relative">
                        <h2 className="text-lg font-semibold mb-4">{editMode ? 'Edit Buyer' : 'Add New Buyer'}</h2>
                        <button
                            onClick={resetForm}
                        ...
                        <form onSubmit={handleSubmit} className="space-y-4">
                            {['companyName', 'email', 'phone', 'address', 'gst'].map((field) => (
                                <div key={field}>
                                    ...
                                    <input
                                        ...
                                        required={['companyName', 'email', 'phone'].includes(field)}
                                    />
                                </div>
                            ))}
